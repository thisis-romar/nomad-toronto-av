...
function ensureLabels() {
  ...
  // Build color map from config.labels if present, fall back to defaults
  const configColors = {};
  for (const l of (config.labels ?? [])) configColors[l.name] = l.color;
  for (const label of needed) {
    if (!existing.has(label)) {
      const color = configColors[label] ?? 'ededed';
      const desc = (config.labels ?? []).find(l => l.name === label)?.description ?? '';
      const descFlag = desc ? `--description "${desc}"` : '';
      ghRaw(`label create "${label}" --repo ${owner}/${repo} --color ${color} ${descFlag}`);
...
for (const item of config.items) {
  ...
  if (existing[item.title]) {
    ...
    // Apply all labels (not just first)
    const labelFlag = (item.labels ?? []).map(l => `--label "${l}"`).join(' ');
    const url = ghRaw(
  ...
  if (itemId) {
    ...
    setField(itemId, fieldIds['owner'],     optMaps['owner']?.[item.owner]);
    setField(itemId, fieldIds['site visit'],optMaps['site visit']?.[item.siteVisit]);
  }
}
