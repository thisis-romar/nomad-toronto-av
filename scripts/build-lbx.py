...
from rack_palette import short_conn, ascii_label  # noqa: E402
...
# Which CSV columns feed each face (mirrors build-cable-labels.py).
...
# What each band carries (mirrors build-cable-labels.py — keep the two in step).
# The middle band is the one the wrap destroys, so it carries the connectors
# rather than a third copy of the identity: nothing is lost that was not already
# lost, and whatever survives is information the other two bands do not have.
#
#   band 0  top     rotated 180   line1 / line2    identity
#   band 1  middle  rotated 180   conn_a / conn_b  connectors
#   band 2  bottom  upright       line1 / line2    identity
#
# Object order therefore runs Text1..Text6 = line1, line2, conn_a, conn_b,
# line1, line2 — which is why the sheet CSVs lead with those four columns.
BAND_FIELDS = (
    ("line1", "line2"),
    ("conn_a", "conn_b"),
    ("line1", "line2"),
...
def resolve(row, field):
    """Sample text for one band slot, matching the proof renderer exactly."""
...
def triple_xml(row):
    """Six objects: three two-line bands, contents per BAND_FIELDS.
    ...
    for band_i, angle in ((2, 0), (1, ROT_180), (0, ROT_180)):
        texts = [resolve(row, f) for f in BAND_FIELDS[band_i]]
        # A rotated band must also be stacked in reverse. The angle flips the
        # glyphs but not the boxes, so leaving the slots in page order makes the
        # face read line 2 above line 1 once the tag is folded. Lay the slots out
        # bottom-to-top instead; objects are still *emitted* in slot order, which
        # is what the merge dialog binds against.
        order = list(range(len(BAND_LAYOUT)))
        if angle:
            order.reverse()
        tops, y = {}, PRINT_Y + band_i * BAND_H_PT + lead
        for slot in order:
            tops[slot] = y
            y += heights[slot] + lead
        for slot, (size_mm, bold) in enumerate(BAND_LAYOUT):
            objs.append(text_object(f"Text{len(objs) + 1}", PRINT_X, tops[slot],
                                    PRINT_W, heights[slot], size_mm * PT, bold,
                                    texts[slot], angle))
...
def label_xml(lines_a, lines_b=None):
    """Face A in the lower half; Face B in the upper half, rotated 180.

    The faces carry different content: identity on A, connectors on B.
    """
    lines_b = lines_a if lines_b is None else lines_b
    layout = LAYOUT_3 if max(len(lines_a), len(lines_b)) >= 3 else LAYOUT_2
    ...
    objs = []
    for i, (box, sz, bold, txt) in enumerate(zip(boxes, sizes, bolds, lines_a)):
        objs.append(text_object(f"Text{i + 1}", *box, sz, bold, txt))
    for i, (box, sz, bold, txt) in enumerate(zip(boxes, sizes, bolds, lines_b)):
        objs.append(text_object(f"Text{len(lines_a) + i + 1}", *rot180_box(*box),
                                sz, bold, txt, ROT_180))
    return (
        '<?xml version="1.0" encoding="UTF-8"?>\n'
        f'<pt:document {NS} version="1.7" generator="{GENERATOR}">'
...
def write_lbx(path, row, lines_a, lines_b, title, note):
    xml = triple_xml(row) if TRIPLE else label_xml(lines_a, lines_b)
...
def main():
    ...
    lines_a = [ascii_label(first.get(f, "")) for f in FACE_A_FIELDS
               if (first.get(f) or "").strip()]
    lines_b = [ascii_label(short_conn(first.get(f, ""))) for f in FACE_B_FIELDS
               if (first.get(f) or "").strip()]
    write_lbx(dst, first, lines_a, lines_b,
    ...
    if TRIPLE:
        bands = "  ".join(
            f"band{i} [{' / '.join(resolve(first, f) for f in slots)}]"
            for i, slots in enumerate(BAND_FIELDS))
        print(f"{dst}: template from {first['id']}  {bands}")
    else:
        print(f"{dst}: template from {first['id']}  "
