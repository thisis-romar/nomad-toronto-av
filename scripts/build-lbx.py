...
LINE1_PT = 4.0 * PT        # ~11.3 pt headline
LINE2_PT = 2.8 * PT        # ~7.9 pt detail
...
def label_xml(line1, line2):
    # Face A occupies the lower half, inside the printable box.
    l1_h, l2_h = LINE1_PT * 1.15, LINE2_PT * 1.15
    a1 = (PRINT_X, FACE_A_TOP + 1.0, PRINT_W, l1_h)
    a2 = (PRINT_X, PRINT_BOT - l2_h - 0.5, PRINT_W, l2_h)
    objs = [
        text_object("Text1", *a1, LINE1_PT, True, line1),
        text_object("Text2", *a2, LINE2_PT, False, line2),
        text_object("Text3", *rot180_box(*a1), LINE1_PT, True, line1, ROT_180),
        text_object("Text4", *rot180_box(*a2), LINE2_PT, False, line2, ROT_180),
    ]
    return (
...
def write_lbx(path, line1, line2, title, note):
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        # lowercase label.xml, and first in the archive, as Editor writes it
        z.writestr("label.xml", label_xml(line1, line2))
...
def main():
    ...
    write_lbx(dst, first["line1"], first["line2"],
    ...
    print(f"{dst}: template built from {first['id']} "
          f"({first['line1']} / {first['line2']})")
    print(f"  printable area {PRINT_W / PT:.2f} x {PRINT_H / PT:.2f} mm "
          f"(NOT 20x20 — Editor's own margins)")
    print(f"  4 text objects; Face B at angle={ROT_180} (degrees assumed — see docstring)")
