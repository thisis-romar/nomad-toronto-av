...
- 🟡 **L9 — Lighting load:** 5.73–5.93 kW ≈ 48–49 A @ 120 V, a subtotal — only the CO₂ jets carry no power figure, and the currents assume unity PF. A 4-branch 120 V/20 A allocation is adopted from EMBLEM Rev 7.2 (verified); the **installed** feed and breakers are still TBC. The hazer, at 4.4 A, sits on a DP-415 channel rated 5 A, and the unmeasured CO₂ jets land on LX-4's 2.81 A margin.
- 🟡 **L13 — Rev 7.2 cites a superseded file:** its sources page links `08-lighting/nomad-lighting-spec-v1.md` on `main`, which is 22 commits behind and still attributes the beams to a Clay Paky Sharpy — a claim Rev 7.2 itself supersedes. Merge the branch or repoint the link. See `08-lighting/rev7-2-reconciliation.md` §4.
...
- 🔴 **L11 — DP-415 Dimmer/Switch mode:** the Chauvet Hurricane Haze 2D must **not** be run on a dimmer, and the DP-415's mode is selected pack-wide by dip switch 10. The venue confirms **one** pack carries the hazer and **both** CO₂ jets (3 of 4 channels), so the hazer cannot be isolated onto its own pack — **dip 10 must read Switch.** Unverified. Read it before the next show.
