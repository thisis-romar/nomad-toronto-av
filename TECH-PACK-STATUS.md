...
- 🟡 **L3 — Generic MA profiles → real models:** *mostly resolved.* Lasers are **Panda Lighting LS650/LS652**, beams **YF BEAM 230**, washes **BETOPPER LM70S**, strobes **Light4Me STROBE MULTI BAR**. Still unknown: `3 LED Bar 2 11CH` and `2 Dimmer 00` (CO₂ ×2 + hazer). The **Clay Paky brand** claim is retracted, though the beams are a Sharpy-class discharge fixture after all. See `08-lighting/fixture-identification-audit.md`.
- 🔴 **L8 — Patch/mode mismatch (×3):** strobes patched 13CH (fixture offers 4/16/168), lasers 26CH (11/19/24), beams 14CH (16/20). The beams' 16-channel address stride matches the fixture rather than the profile, so that one is a wrong-profile error with no collision; the strobes collide if they are really in 16CH. No address overlaps as patched, so **do not re-patch** until the panel check in audit §7 is done.
- 🟡 **L9 — Lighting load:** 5.15–5.35 kW identified ≈ 43–45 A @ 120 V, a subtotal — DJ bar, CO₂ jets and hazer carry no power figure, and the currents assume unity PF. Mains feed and breakers still TBC.
